...
- **Двухшаговый трекинг задач:** Корректно подписывается на задачи компиляции и прошивки (`firmware/follow_job`) и дожидается их завершения (`event: result`).
...
- **Связь с ESPHome:** Библиотека `websockets`. Запросы отправляются в виде JSON-RPC сообщений, совместимых с новым протоколом ESPHome Device Builder API (обертка параметров в `args`, ожидание события `event: result`).
