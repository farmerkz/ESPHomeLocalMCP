...
- **Связь с ESPHome:** WebSocket `ws://<host>:<port>/ws`.
- **Конфигурация:** Настройки хоста и порта считываются из файла `.env` в формате TOML (`host`, `port`). При отсутствии файла или ошибках парсинга используется fallback на `localhost` и порт `6052`.
